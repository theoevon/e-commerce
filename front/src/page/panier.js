...
const Panier = () => {
    ...
    const SupprimerToutPanier = () => {
        ...
        for (let i = 0; i < compteur; i++) {
            document.getElementById("article" + tab[i]).remove();
        }
        window.localStorage.removeItem("article_add");
        document.getElementsByClassName("container_panier")[0].innerHTML = "<h1>PANIER VIDE</h1>";
    }
    ...
    const SupprimerElementPanier = (e) => {
        ...
        let value_local = window.localStorage.getItem("article_add");
        let tab = value_local.split(',');
        let compteur1 = tab.length;
        let arr1 = [];
        for (let i = 0; i < compteur1; i++) {
            if (tab[i] !== id) {
                arr1.push(tab[i]);
            }
        }
        window.localStorage.setItem("article_add", arr1);
                            ...
                            <table>
                                ...
                                <tbody>
                                    {articles.map((article) => {
                                        ...
                                        return <tr className='border-top' id={"article" + article.id}>
                                            ...
                                            <td><img src="https://cdn-icons-png.flaticon.com/512/251/251319.png" alt="croix" className="croix" id={"/" + article.id} onClick={(e) => SupprimerElementPanier(e)}></img></td>
                                        </tr>
                                    })}
                                </tbody>
                            </table>
